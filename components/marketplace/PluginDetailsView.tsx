...
function PluginDetailsContent({ slug }: PluginDetailsViewProps) {
    ...
    const breadcrumbSchemaData = generateBreadcrumbSchema([
        ...
        { name: plugin.name, item: `/marketplace/${plugin.slug}` },
    ]);

    // 3. Plugin Loaded View
    return (
        <div className="w-full min-h-screen bg-[#07071a] text-slate-200 pb-0 md:pb-20 pt-28 sm:pt-36">
            {/* Embedded SoftwareApplication & BreadcrumbList JSON-LD Schemas */}
            <script
                type="application/ld+json"
                dangerouslySetInnerHTML={{
                    __html: JSON.stringify([pluginSchemaData, breadcrumbSchemaData]),
                }}
            />
